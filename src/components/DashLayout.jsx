...
import {
  Phone, PhoneIncoming, Activity, Bot, BarChart3, Settings,
  LogOut, ChevronRight, Zap, TrendingUp, GitBranch, Menu, X
} from 'lucide-react'
    ...
    <div className="flex min-h-screen bg-ink">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-30 flex items-center justify-between px-4 h-14 bg-surface border-b border-border">
        <Link to="/" className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-md bg-lime flex items-center justify-center flex-shrink-0">
            <svg width="12" height="12" viewBox="0 0 20 21" fill="none">
              <rect x="1.5"  y="6"   width="2.5" height="9"  rx="1.25" fill="#0A0A0F"/>
              <rect x="5"    y="3.5" width="2.5" height="14" rx="1.25" fill="#0A0A0F"/>
              <rect x="8.5"  y="1.5" width="2.5" height="18" rx="1.25" fill="#0A0A0F"/>
              <rect x="12"   y="3.5" width="2.5" height="14" rx="1.25" fill="#0A0A0F"/>
          ...
          <span className="font-display font-bold text-sm text-cream">speekeasy</span>
        </Link>
        <button
          onClick={() => setDrawerOpen(true)}
          className="p-2 rounded-lg text-ghost hover:text-cream hover:bg-muted transition-all"
      ...
      {/* Mobile drawer */}
      <div className={clsx(
        'lg:hidden fixed top-0 left-0 bottom-0 z-50 w-64 transition-transform duration-200 ease-in-out',
        drawerOpen ? 'translate-x-0' : '-translate-x-full'
      )}>
        <div className="relative h-full">
          <button
            onClick={() => setDrawerOpen(false)}
            className="absolute top-4 right-3 p-1.5 rounded-lg text-ghost hover:text-cream hover:bg-muted transition-all z-10"
      ...
      {/* Main content — padded top on mobile for hamburger bar */}
